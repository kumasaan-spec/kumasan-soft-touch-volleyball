...
const CANDIDATE_WINDOW_SIZE = 24
const FALLBACK_WINDOW_SIZE = 56
const COMPLETION_SEARCH_LIMIT = 16
...
const FOUR_COURTS = ['A', 'B', 'C', 'D'] as const
const DEFAULT_VENUE_NAME = '会場1'

type FourCourtId = (typeof FOUR_COURTS)[number]
...
type TeamScheduleState = {
  ...
  lastCourt: FourCourtId | null
  lastVenueName: string | null
  previousVenueName: string | null
  courtCounts: Record<FourCourtId, number>
...
const createVenueLookup = (courtVenues: CourtVenueSetting[]) => {
  const lookup = new Map<FourCourtId, string>()

  courtVenues.forEach((setting) => {
    if (setting.court === 'A' || setting.court === 'B' || setting.court === 'C' || setting.court === 'D') {
...
const canPartitionIntoFullSlots = (
  matches: PendingMatch[],
  matchCount: number,
  ...
  const memoKey = matches.map((match) => match.id).join(',')
  if (failedKeys.has(memoKey)) {
    return false
  }
  ...
  const search = (startIndex: number): boolean => {
    if (selectedIndexes.length === matchCount) {
      const selectedIndexSet = new Set(selectedIndexes)
      const remainingMatches = matches.filter(
        (_, matchIndex) => !selectedIndexSet.has(matchIndex),
      )

      return canPartitionIntoFullSlots(remainingMatches, matchCount, failedKeys)
    }

    for (let index = startIndex; index < matches.length; index += 1) {
      const nextMatch = matches[index]
      const selectedMatches = selectedIndexes.map((selectedIndex) => matches[selectedIndex])

      if (hasAnyTeamOverlap([...selectedMatches, nextMatch])) {
        continue
      }
...
const scoreCourtForTeam = (
  ...
  const nextCourtCounts = FOUR_COURTS.map((targetCourt) =>
...
const createCourtPlanOptions = (
  selectedIndexes: number[],
  availableCourts: readonly FourCourtId[] = FOUR_COURTS,
): CourtPlan[][] => {
...
const scoreCourtPlans = (
  ...
  pendingMatches: PendingMatch[],
  teamStates: TeamScheduleState[],
  venueLookup: Map<FourCourtId, string>,
  previousCourtTeamIndexes: Map<FourCourtId, CourtTeamIndexes | null>,
) => {
...
const findBestPlanForMatchCount = (
  ...
  const search = (startIndex: number): void => {
    ...
    for (let index = startIndex; index < windowSize; index += 1) {
      const nextMatch = pendingMatches[index]
      const selectedMatches = selectedIndexes.map((selectedIndex) => pendingMatches[selectedIndex])
...
const createEmptyAssignment = (court: FourCourtId): CourtAssignment => ({
  court,
  match: null,
})

const createInitialPreviousCourtTeamIndexes = () =>
  new Map<FourCourtId, CourtTeamIndexes | null>(
    FOUR_COURTS.map((court) => [court, null]),
  )
...
const updatePreviousCourtTeamIndexes = (
  ...
  FOUR_COURTS.forEach((court) => previousCourtTeamIndexes.set(court, null))
...
const getCourtCode = (court: FourCourtId) => FOUR_COURTS.indexOf(court) + 1

const createVenueCodeLookup = (venueLookup: Map<FourCourtId, string>) => {
  const codeByVenueName = new Map<string, number>()
  const codeByCourt = new Map<FourCourtId, number>()

  FOUR_COURTS.forEach((court) => {
...
const createAlwaysActiveCourtPlanSlots = (
  ...
  const initialState: AlwaysActiveCourtState = {
    ...
    lastCourtTeamIndexes: FOUR_COURTS.map(() => null),
  ...
  for (const selectedMatches of matchSlots) {
    ...
    for (const state of states.values()) {
      for (const courtPlans of courtPlanOptions) {
        ...
        const nextLastCourtTeamIndexes: Array<CourtTeamIndexes | null> = FOUR_COURTS.map(() => null)
...
const updateScheduleState = (
  ...
  const courtByTeamIndex = new Map<number, FourCourtId>()
...
const calculateScheduleQuality = (
  ...
  const teamStats = new Map(
    teams.map((team) => [
      team.id,
      {
        ...
        venueMoves: 0,
        venueRoundTrips: 0,
        lastCourt: null as FourCourtId | null,
  ...
  for (const slot of slots) {
    ...
    const courtByTeamId = new Map<string, FourCourtId>()
    ...
    for (const assignment of slot.courts) {
      ...
      const venueName = venueLookup.get(assignment.court as FourCourtId) ?? DEFAULT_VENUE_NAME
      courtByTeamId.set(assignment.match.teamA.id, assignment.court as FourCourtId)
      courtByTeamId.set(assignment.match.teamB.id, assignment.court as FourCourtId)
...
const createNextRestLockedState = (
  ...
  futureCardAvailabilityCounts: number[][],
  venueCodeLookup: Map<FourCourtId, number>,
...
const findRestLockedNoImmediateCourtPlanSlots = (
  ...
  if (
    activeTeamIndexesBySlot.some(
      (activeTeamIndexes) => activeTeamIndexes.length !== FOUR_COURTS.length * 2,
    )
  ) {
    return null
  }
...
const buildRestLockedScheduleSlots = (
  ...
  sourceSlots.map((sourceSlot, slotIndex) => {
    const courts = FOUR_COURTS.map((court) => {
...
export const generateFourCourtSchedule = ({
  courtCount,
  roundCount,
  teamNames,
  courtVenues,
  ...
  if (courtCount !== 4) {
    throw new Error('4コートの組み合わせ生成には、コート数を3にしてください。')
  }
  ...
  if (teamNames.length < 8) {
    throw new Error('4コートでは6チーム以上を指定してください。')
  }
